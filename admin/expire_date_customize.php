                ...
                <header class="card-header">
                    ...
                    <div class="row">
                        <div class="col-lg-10" style="padding-right: 0">
                            <h2 class="card-title pt-2">Stock Summery</h2>
                        </div>
                ...
                <div class="card-body" id="rangeTotalSales">
                    <table class="table table-bordered table-striped mb-0" id="datatable-tabletools3">
                        ...
                        while ($row = mysqli_fetch_assoc($items)) {
                            $total = 0;
                            $branch1 = find_all_branch();
                            ?>
                            <tr>
                                <td><?php echo $k++; ?></td>
                                <td><?php echo $row["code"] . "/" . $row["title"]; ?></td>
                                <?php while ($row1 = mysqli_fetch_assoc($branch1)) {
                                    $stock = find_total_inventory_warehouse_item($row1["id"], $row["id"]);
        ...
        <div class="col-md-12" style="margin-bottom: 15px">
            <section class="card">
                <header class="card-header">
                    ...
                    <div class="row">
                        <div class="col-lg-3">
                            <h2 class="card-title pt-2">Expire date</h2>
                        </div>

                        <div class="col-lg-4 row">
                            <label class="col-md-4 control-label text-lg-right pt-2">Branch</label>
                            <select name="branch01" id="branch01" class="populate col-md-8" onchange="searchExpireDate()">
                                <?php
                                echo "<option value=''>All Branch's</option>";
                                while ($row = mysqli_fetch_assoc($branchs01)) {
                                    ?>
                                    <option
                                            value='<?php echo $row["id"]; ?>'><?php echo $row["title"]; ?></option>
                                <?php } ?>
                        ...
                        <div class="col-lg-1">
                            <button class="btn btn-success" id="searchTotalData" onclick="searchExpireDate()"
                                    style="display: block">Search
                            </button>
                        </div>
                            ...
                            <tr>
                                ...
                                <!-- expiered_date -->
                                    ...
                                    <?php echo ($ED > $CD) ? "" : "<span style='border-radius: 5px; background: red;padding: 5px 10px;color: white;'>Expired</span>";
                                     ?>
                                      <!-- <?php echo ($LD=0) ? "" : "<span style='border-radius: 5px; background: yellow;padding: 5px 10px;color: white;'>Expire Today</span>";
                                    $LD ?> -->
                                </td>
                            </tr>
                        <?php } ?>
...
<script>
    $(document).ready(function () {
        $('#datatable-tabletools3').DataTable({
            dom: 'Bflrtip',
            "displayLength": 10,
            buttons: [
                'excel', 'pdf', 'print'
...
<!-- Examples -->
<style>
    #datatable-tabletools3_filter, #datatable-tabletools3_length, #datatable-tabletools4_filter, #datatable-tabletools4_length, #datatable-tabletools5_filter, #datatable-tabletools5_length, #datatable-tabletools6_filter, #datatable-tabletools6_length, #datatable-tabletools05_filter, #datatable-tabletools05_length {
        float: right;
    }
