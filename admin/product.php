                    ...
                    <table class="table table-bordered table-striped mb-0" id="datatable-tabletools">
                        ...
                        while ($row = mysqli_fetch_assoc($product)) {
                            ...
                            <tr>
                                ...
                                <!--                                <td>-->
                                <?php //echo (find_category_by_id($row["category"])['category']="1")?"Finished Product":"Raw Material"; ?><!--</td>-->
                                <td><?php echo $row["measurment"]; ?></td>
                                <td><?php echo $row["min_stock"] . " " . $row["measurment"]; ?></td>
                                <td><?php echo $row["min_shop"] . " " . $row["measurment"]; ?></td>
                                <td><?php echo $row["min_showroom"] . " " . $row["measurment"]; ?></td>
                                <td><?php echo find_user_by_id($row["created_by"])["name"]; ?></td>
                                <td class="actions">
                                    <a href="#" class="on-editing save-row p-1"><i class="fa fa-info"></i></a>
                                    <a href="#" class="hidden on-editing cancel-row p-1"><i class="fa fa-times"></i></a>
                                    <a href="editProduct.php?id=<?php echo urldecode($row["id"]); ?>"
                                       class="on-default edit-row p-1"><i class="fa fa-pencil"></i></a>
                                    <a href="deleteProduct.php?id=<?php echo urldecode($row["id"]); ?>"
