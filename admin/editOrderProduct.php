...
if (isset($_POST["submit"])) {
    if (isset($_POST["title"]) && isset($_POST["attribute"])) {
        ...
        $attr = json_encode($_POST["attribute"]);
        $measur = json_encode($_POST["measurement"]);

        $query = "UPDATE order_product SET ";
        $query .= "title = '{$title}'";
        $query .= ",attribute = '{$attr}' ";
        $query .= ",measurment = '{$measur}' ";
        $query .= "WHERE id = {$id}; ";
        ...
        <div class="col">
            <section class="card">
                <header class="card-header">
                    ...
                    <h2 class="card-title">Update Order Product</h2>
                </header>
                <div class="card-body">
                    ...
                    <form class="form-horizontal form-bordered" action="#" method="post" enctype='multipart/form-data'>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-lg-4">
                                <input type="text" name="title" class="form-control" data-plugin-maxlength
                                       maxlength="100" value="<?php echo $product["title"] ?>" required/>
                            ...
                            <div class="col-lg-4">
                                <select name="attribute[]" data-plugin-selectTwo class="form-control populate"
                                    ...
                                    while ($row = mysqli_fetch_assoc($attribute)) {
                                        ?>
                                        <option <?php echo (sizeof(json_decode($product["attribute"])) > 0) ? in_array($row["id"], json_decode($product["attribute"])) ? "selected" : "" : ""; ?>
                        ...
                        <div class="form-group row">
                            <label class="col-lg-2 control-label text-lg-right pt-2">Measurement</label>

                            <div class="col-lg-4">
                                <select name="measurement[]" data-plugin-selectTwo class="form-control populate"
                                        multiple>
                                    <?php
                                    while ($row = mysqli_fetch_assoc($measurement)) {
                                        ?>
                                        <option <?php echo (sizeof(json_decode($product["measurment"])) > 0) ? in_array($row["id"], json_decode($product["measurment"])) ? "selected" : "" : ""; ?>
                                            value="<?php echo $row["id"] ?>"><?php echo $row["title"]; ?></option>
                                    <?php } ?>
                                </select>
                            </div>
